...
1. 渲染管线总结
   ...
   - Forward Rendering  
      ...
      - 遍历物体，再遍历光源，或者遍历光源，再遍历物体；时间复杂度 ： M * N , 因此增加光源数目，就会导致渲染效率下降；
...
2. 环境光遮罩
   ...
   - Precomputed AO：离线预处理的环境光贴图，适用于静态物体；

   - Screen Space Ambient Occlusion(SSAO) 屏幕空间环境光遮罩:可以解决动态物体的遮罩效果；
     ...
     - **仅基于2D屏幕像素信息推算遮蔽关系，主要使用每个像素的深度缓冲数据和法线缓冲数据**

     - 计算过程：

       - 根据像素P的深度和相机投影矩阵，反算其世界坐标位置
       - 在P的法线方向半球，生成随机采样点S
       - 每个采样点S转换（view+projection+透视除法）到屏幕空间S‘，
       - S'的深度与深度缓冲值比较
...
6. **Frame Graph（帧图）** 是一种**声明式、数据驱动的渲染管线管理架构**。它的核心目标是**显式地声明、优化和管理一帧渲染所需的所有渲染通道及其资源依赖关系**，从而解决复杂渲染管线中的资源管理、同步和扩展性问题。