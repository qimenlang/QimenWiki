...
2. heightfeild : 高程图，等高线图
   ...
   - 高程图：每隔一定距离，做均匀的网格，根据高程图，做一定的位移；
...
10. parallax and displacement mapping 
    ...
    - Parallax Mapping: 根据高度，扰动UV,Due to the height of the surface, the eye sees point B instead of point A. It creates a sense of dimensionality；

    - Displacement mapping : 根据高度，直接对mesh 顶点进行变形；
...
13. DirectStorage : 数据从硬盘到内存，从内存到显存，传输的是压缩数据；在显存中解压缩；

14. DMA(Direct Memory Access) : 显卡直接加载硬盘数据到显存，不走内存；

15. 解决float 精度引起的z-fighting问题

    - Camera-Relative Rendering:相机相对性的渲染，把相机的位置强制设定为零，其他的物体在世界坐标系的位置转换为相对相机的位置；

16. **Road and Decals Rendering**
