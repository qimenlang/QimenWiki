...
static bool insideTriangle(float x, float y, const Vector4f* _v){
    ...
    f1 = v[2].cross(v[1]);
    f2 = v[0].cross(v[2]);
    Vector3f p(x,y,1.);//加上0.5才是像素的真正中心
...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos)
{
	...
	bool use_msaa = false;
	std::vector<Vector2f> MSAA_Sample = { {0.25f,0.25f},{0.25f,0.75f},{0.75f,0.25f},{0.75f,0.75f} };
	...
	if (use_msaa) {
		for (int x = bb_min.x(); x < bb_max.x(); x++)
		{
			for (int y = bb_min.y(); y < bb_max.y(); y++)
			{
				...
				for (auto sample : MSAA_Sample)
				{
					if (insideTriangle(x + sample.x(), y + sample.y(), t.v)) {
						...
						msaa_sample_count++;
						if (depth > z_interpolated) depth = z_interpolated;
					}
				}
				if (depth_buf[get_index(x, y)] > depth)
				{
					//Vector3f color = t.getColor() * msaa_sample_count / 4.f;
					//depth_buf[get_index(x, y)] = depth;

					//set_pixel({ x,y }, frag_color);
				}
			}
		}
	}
	...
	for (int x = x_min; x < x_max; x++)
	{
		for (int y = y_min; y < y_max; y++)
		{
			if (insideTriangle(x+0.5, y+0.5, t.v))
			{
				...
				fragment_shader_payload shader_payload(interpolated_color, interpolated_normal, interpolated_texcoords, diffuse_texture ? &*diffuse_texture : nullptr,bump_texture?&*bump_texture:nullptr);
				shader_payload.view_pos = interpolated_shadingcoords;

				Vector3f frag_color = fragment_shader(shader_payload);
...
rst::rasterizer::rasterizer(int w, int h) : width(w), height(h)
{
	...
	bump_texture = std::nullopt;
	displacement_texture = std::nullopt;
}
